...
import numpy as np
from ..core.backend import xp, to_device
...
def cross_entropy_loss(
    ...
    targets_flat = to_device(targets.reshape(-1))  # (batch*seq,)
    ...
    log_sum_exp = xp.log(xp.sum(xp.exp(logits_shifted), axis=-1, keepdims=True))
    log_probs = logits_shifted - log_sum_exp  # (batch*seq, vocab)

    # Create mask for valid positions
    mask = (targets_flat != ignore_index).astype(xp.float32)
    valid_count = xp.sum(mask)

    if valid_count == 0:
        return Tensor(xp.array(0.0), requires_grad=False)

    # Get log probability of correct tokens
    # Use advanced indexing: for each position, get log_prob of target token
    indices = xp.arange(len(targets_flat))
    valid_targets = xp.where(targets_flat == ignore_index, 0, targets_flat)
    correct_log_probs = log_probs[indices, valid_targets]  # (batch*seq,)

    # Apply mask and compute mean loss
    masked_log_probs = correct_log_probs * mask
    loss_value = -xp.sum(masked_log_probs) / valid_count
    ...
    def _backward():
        if logits.requires_grad:
            ...
            softmax_probs = xp.exp(log_probs)  # (batch*seq, vocab)
