...
def relu(x: Tensor) -> Tensor:
    ...
    def _backward():
        if x.requires_grad:
            ...
            grad = (x.data > 0).astype(_backend.xp.float32) * out.grad
...
def gelu(x: Tensor) -> Tensor:
    ...
    sqrt_2_over_pi = float(_backend.xp.sqrt(2.0 / _backend.xp.pi))
    x_cubed = x.data ** 3
    inner = sqrt_2_over_pi * (x.data + 0.044715 * x_cubed)
    tanh_inner = _backend.xp.tanh(inner)
...
def softmax(x: Tensor, axis: int = -1) -> Tensor:
    ...
    x_max = _backend.xp.max(x.data, axis=axis, keepdims=True)
    exp_x = _backend.xp.exp(x.data - x_max)
    sum_exp = _backend.xp.sum(exp_x, axis=axis, keepdims=True)
    ...
    def _backward():
        if x.requires_grad:
            s = result
            sum_grad = _backend.xp.sum(out.grad * s, axis=axis, keepdims=True)
...
def log_softmax(x: Tensor, axis: int = -1) -> Tensor:
    ...
    log_sum_exp = _backend.xp.log(_backend.xp.sum(_backend.xp.exp(shifted), axis=axis, keepdims=True))
    ...
    def _backward():
        if x.requires_grad:
            softmax_result = _backend.xp.exp(result)
            sum_grad = _backend.xp.sum(out.grad, axis=axis, keepdims=True)
...
if __name__ == "__main__":
    ...
    print(f"Sum of softmax = {_backend.xp.sum(y_softmax.data):.4f} (should be 1.0)")
